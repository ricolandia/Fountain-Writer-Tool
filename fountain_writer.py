#!/usr/bin/env python3
"""
Fountain Writer — Fountain Screenplay Editor

Features:
  - Fountain syntax highlighting (scenes, characters, dialogues, transitions)
  - Find and replace (Ctrl+H) with navigation between results
  - Word / character count / estimated duration in the status bar
  - Scene navigator in the sidebar
  - Focus mode (F11) — hides toolbar/sidebar, centers the paper
  - Line numbers with visual page separator (55 lines)
  - Character autocomplete when typing in UPPERCASE
  - Current line highlight
  - Export to PDF (requires reportlab)
  - Light/dark themes
  - Persistent configuration (theme, geometry, preferences)
"""
...
class HoverButton(tk.Button):
    """Button with hover effect — keyboard accessible."""
...
class FindReplaceDialog(tk.Toplevel):
    ...
    Always stays visible over the editor without blocking editing.
    """

    def __init__(self, master, text_widget, theme_colors):
        super().__init__(master)
        self._text    = text_widget
        self._matches = []   # list of (start, end) results
        self._current = -1   # index of the highlighted result

        self.title("Find / Replace")
        self.resizable(False, False)
        self.protocol("WM_DELETE_WINDOW", self._fechar)

        # Position to the top right of the parent
        ...
        tk.Label(row1, text="Find:", width=11, anchor="w", **lbl_kw).pack(side="left")
        ...
        tk.Label(row2, text="Replace:", width=11, anchor="w", **lbl_kw).pack(side="left")
        ...
        tk.Button(row4, text="◀ Previous", command=self._anterior,
                  bg=c["sidebar_sel"], fg="#fff", **btn_kw).pack(side="left", padx=2)
        tk.Button(row4, text="Next ▶", command=self._proximo,
                  bg=accent, fg="#fff", **btn_kw).pack(side="left", padx=2)
        tk.Button(row4, text="Replace", command=self._substituir,
                  bg=c["sidebar"], fg=c["sidebar_fg"],
                  relief="groove", **btn_kw).pack(side="left", padx=(12, 2))
        tk.Button(row4, text="Replace all", command=self._substituir_tudo,
                  bg=c["sidebar"], fg=c["sidebar_fg"],
    ...
    def _buscar_auto(self):
        """Triggers search while typing (without moving cursor)."""
    ...
    def _buscar(self, mover=True):
        ...
        if total == 0:
            self._lbl_count.config(text="No results")
            self._current = -1
            return
...
class FountainEditor:
    ...
    def __init__(self, root):
        ...
        # Monospace font (cross-platform fallback)
    ...
    def _build_ui(self):
        ...
        self._add_toolbar_btn("☰  Scenes",  self._toggle_sidebar)
        self._add_toolbar_btn("📂  Open",  self._carregar_arquivo)
        self._add_toolbar_btn("💾  Save", self._salvar_arquivo)
        self._add_toolbar_btn("📄  New",   self._novo_arquivo)
        self._add_toolbar_btn("📑  PDF",    self._exportar_pdf)
        self._add_toolbar_btn("🔍  Search", self._abrir_busca)
        self._add_toolbar_btn("🎯  Focus",   self._alternar_foco)
        self._add_toolbar_btn("🌗  Theme",   self._alternar_tema)
        self._add_toolbar_btn("❓  Help",  self._mostrar_ajuda)
        self._add_toolbar_btn("✕  Exit",   self._sair,
        ...
        self._lbl_scenes = tk.Label(
            self._status_bar, text="Scenes: 0", font=("Segoe UI", 9))
        self._lbl_scenes.pack(side="left", padx=16, pady=2)

        self._lbl_words = tk.Label(
            self._status_bar, text="Words: 0", font=("Segoe UI", 9))
        self._lbl_words.pack(side="left", padx=4, pady=2)

        self._lbl_chars = tk.Label(
            self._status_bar, text="| Characters: 0", font=("Segoe UI", 9))
        ...
        self._lbl_page = tk.Label(
            self._status_bar, text="Page: 1", font=("Segoe UI", 9))
    ...
    def _escape_handler(self, _=None):
        """Escape: close autocomplete or exit focus mode."""
    ...
    def _carregar_arquivo(self):
        ...
        fp = filedialog.askopenfilename(
            ...
            filetypes=[("Fountain", "*.fountain"), ("Text", "*.txt"),
                       ("All", "*.*")])
        if not fp:
            return
    ...
    def _feedback_salvo(self):
        self._lbl_saved.config(text="✓ Saved", fg="#4caf50")
    ...
    def _alternar_foco(self, _=None):
        """
        Focus mode: hides toolbar and sidebar, increases side padding
        to simulate an isolated sheet. F11 or Esc restores.
        """
        ...
        if self._focus_mode:
            ...
            self._lbl_saved.config(
                text="Focus mode  —  F11 or Esc to exit",
                fg=c["statusbar_fg"])
            self.root.after(3000, lambda: self._lbl_saved.config(text=""))
        else:
            # Restore toolbar before _main
    ...
    def _atualizar_contador_palavras(self):
        """
        Counts words and characters. Estimates script duration
        (standard: ~1 min per page, where 1 page ≈ 55 lines).
        """
        conteudo = self._text.get("1.0", tk.END)
        ...
        self._lbl_words.config(text=f"Words: {fmt(palavras)}")
        self._lbl_chars.config(text=f"| Characters: {fmt(chars)}")
        self._lbl_readtime.config(
            text=f"| Est. duration: {tempo}" if tempo else "")

    def _agendar_contagem(self):
        """Debounce of 500 ms to avoid counting on every key press."""
    ...
    def _renderizar(self):
        ...
        self._lbl_scenes.config(text=f"Scenes: {len(new_titles)}")
    ...
    def _atualizar_status(self, _=None):
        try:
            ...
            self._lbl_page.config(
                text=f"Page: {page}  |  Line: {line_num}")
        except Exception:
            pass
    ...
    def _redraw_line_numbers(self):
        ...
        while True:
            ...
            # Draw page separator every 55 lines
    ...
    # -----------------------------------------------------------------
    #  SCROLL BY PAGE (55 lines)
    # -----------------------------------------------------------------
    def _scroll_page(self, direction):
        """direction: -1 (previous), +1 (next) — by block of 55 lines"""
        try:
    ...
    def _toggle_sidebar(self):
        if self._focus_mode:
            return  # sidebar locked in focus mode
        if self._is_sidebar_visible:
            self._sidebar.pack_forget()
        else:
            self._sidebar.pack(side="left", fill="y", before=self._desk)
    ...
    def _exportar_pdf(self):
        if not HAS_PDF_SUPPORT:
            messagebox.showinfo(
                "PDF not available",
                "To export PDF, install the reportlab library.\n\n"
        ...
        try:
            ...
            cv.drawText(tobj)
            cv.save()
            messagebox.showinfo("PDF exported", f"File saved to:\n{fp}")
        except Exception as e:
            messagebox.showerror("Error exporting PDF", str(e))
    ...
    def _mostrar_ajuda(self):
        ...
        win.title("Help — Fountain Writer")
        ...
        # Shortcuts
        lbl("Keyboard Shortcuts", ("Segoe UI", 11, "bold"), "#569cd6", pady=(0, 8))
        shortcuts = [
            ("Ctrl + S",     "Save file"),
            ("Ctrl + O",     "Open file"),
            ("Ctrl + N",     "New file"),
            ...
            ("F11",          "Activate / exit focus mode"),
            ("Esc",          "Exit focus mode"),
            ("Ctrl + ↑/↓",  "Scroll one page (55 lines)"),
            ("Ctrl + Z",     "Undo"),
            ("Ctrl + Y",     "Redo"),
        ]
        ...
        # Syntax
        lbl("Fountain Syntax", ("Segoe UI", 11, "bold"), "#dcdcaa", pady=(0, 8))
        syntax = [
            ("SCENE",        "INT or EXT followed by description"),
            ("CHARACTER",    "Line in UPPERCASE (e.g., ANA)"),
            ("DIALOGUE",     "Line directly below character"),
            ("PARENTHETICAL", "Between parentheses — e.g., (smiling)"),
            ("TRANSITION",   "Uppercase ending with ':' — CUT TO:"),
            ("CENTER",       "Text between > and <  — e.g., > THE END <"),
        ]
        for title, desc in syntax:
        ...
        # PDF
        lbl("Export PDF", ("Segoe UI", 11, "bold"), "#c586c0", pady=(0, 6))
        lbl("To enable PDF export, run in terminal:",
